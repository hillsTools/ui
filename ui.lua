...
local function InitScreen()
	...
	if IsMobile() then
		...
		for _, obj in pairs(ScreenAsset:GetDescendants()) do
			if obj:IsA("TextButton") or obj:IsA("TextBox") then
				obj.Size = obj.Size + UDim2.new(0, 0, 0, 4) -- Make buttons taller for touch
			end
		end
	end
	
	return {ScreenAsset = ScreenAsset, ToggleButton = ToggleButton}
end
...
local function InitWindow(ScreenAsset,Window)
	...
	-- Make UI mobile-friendly
	if IsMobile() then
		-- Make window larger for mobile
		WindowAsset.Size = UDim2.new(0.8, 0, 0.7, 0)
		WindowAsset.Position = UDim2.new(0.1, 0, 0.15, 0)
		
		-- Make elements larger for touch
		for _, obj in pairs(WindowAsset:GetDescendants()) do
			if obj:IsA("TextButton") or obj:IsA("TextBox") then
				obj.Size = obj.Size + UDim2.new(0, 0, 0, 6)
				obj.TextSize = obj.TextSize + 2
			end
		end
	end
	...
	if ScreenAsset.ToggleButton then
		ScreenAsset.ToggleButton.MouseButton1Click:Connect(function()
			WindowAsset.Visible = not WindowAsset.Visible
		end)
	end
...
local function InitTab(ScreenAsset,WindowAsset,Window,Tab)
	...
	-- Make tab content scrollable for mobile
	if IsMobile() then
		TabAsset.ScrollingEnabled = true
		TabAsset.CanvasSize = UDim2.new(0, 0, 2, 0)
	end
...
local function InitSlider(Parent,ScreenAsset,Window,Slider)
	...
	-- Make slider fatter for mobile
	if IsMobile() then
		SliderAsset.Background.Size = UDim2.new(1, 0, 0, 20) -- Fatter slider
		SliderAsset.Size = UDim2.new(1, 0, 0, SliderAsset.Title.TextBounds.Y + 24) -- More height for fatter slider
	end
	...
	SliderAsset.Title:GetPropertyChangedSignal("TextBounds"):Connect(function()
		...
		-- Adjust for mobile
		if IsMobile() then
			SliderAsset.Size = UDim2.new(1, 0, 0, SliderAsset.Title.TextBounds.Y + 24)
		end
	end)
